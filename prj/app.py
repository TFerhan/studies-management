...
from forms import AjtEleveForm, Achtcours, LoginForm, MDPOUB, Ajtcours, Selectgrp, AchtSeanceForm, Filtrer, filtr, searchelv
...
#         print("lolo")
#         jour = form.jour.data
#         debut = form.debut.data
#         fin = form.fin.data
#         classe = form.classe.data
#         result = []
#         nomcours = []
#         print(form.jour.data, debut, fin, classe)
#         cur = mysql.connection.cursor()
#         cur.execute('select s.idgroupe, g.nombre_eleves from séance s, groupe g where s.jour = %s and s.idgroupe = g.idgroupe', (jour,))
#         gjour = cur.fetchall()
#         if gjour :
#             result.append(gjour)
#         cur.execute('select s.idgroupe, g.nombre_eleves from séance s, groupe g where s.debut = %s and s.idgroupe = g.idgroupe', (debut,))
#         gdebut = cur.fetchall()
#         print(gdebut)
#         if gdebut:
#             result.append(gdebut)
#         cur.execute('select s.idgroupe, g.nombre_eleves from séance s, groupe g where s.fin = %s and s.idgroupe = g.idgroupe', (fin,))
#         gfin = cur.fetchall()
#         if gfin:
#             result.append(gfin)
#         cur.execute('select s.idgroupe, g.nombre_eleves from séance s, groupe g where s.classe = %s and s.idgroupe = g.idgroupe', (classe,))
#         gclasse = cur.fetchall()
#         if gclasse:
#             result.append(gclasse)
#         if not result:
#             flash('Aucun groupe trouvé', 'error')
#             return redirect(url_for('groupes'))
#         groupe_data = result[0]
